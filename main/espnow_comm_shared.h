...
struct CommPacket {
    ...
    uint8_t  version;
    uint8_t  targetId;
    uint32_t txButtonPressMicros;       // [추가] 버튼이 눌린 시점의 송신부 micros() 타임스탬프
    uint32_t txMicros;
    uint32_t delayMs;
    uint32_t playMs;
    uint32_t lastKnownRttUs; // [새로 추가됨] 송신기가 알고 있는 마지막 RTT (마이크로초)
    uint32_t lastKnownRxProcessingTimeUs; // [새로 추가됨] 수신기가 알고 있는 마지막 처리 시간 (마이크로초)
    uint8_t  crc8;
};
...
// CommPacket 크기는 lastKnownRxProcessingTimeUs 추가 및 txButtonPressMicros 추가로 인해 변경됨
static_assert(sizeof(CommPacket) == 31, "CommPacket size mismatch"); // 이전 27 + 4 = 31 bytes
...
//---------------------------------------------------------------------
//  송신부 헬퍼 함수 (CommPacket에 txButtonPressMicros 인자 추가)
//---------------------------------------------------------------------
inline void fillPacket(CommPacket &pkt, uint8_t tgtId, uint32_t txButtonPressMicros, uint32_t delayMs, uint32_t playMs, uint32_t lastRttUs, uint32_t lastKnownRxProcessingTimeUs) {
    ...
    pkt.targetId       = tgtId;
    pkt.txButtonPressMicros = txButtonPressMicros; // [추가]
    pkt.txMicros       = micros();
    pkt.delayMs        = delayMs;
    pkt.playMs         = playMs;
    pkt.lastKnownRttUs = lastRttUs; // [새로 추가됨] 마지막 RTT 정보 추가
    pkt.lastKnownRxProcessingTimeUs = lastKnownRxProcessingTimeUs; // [새로 추가됨] 마지막 수신기 처리 시간 추가
...
inline bool verifyCommPacket(const uint8_t* data, size_t len, const CommPacket*& pkt, uint8_t myId, bool &forMe) {
    // [수정] CommPacket 크기가 31이므로, len >= 31 인지 확인해야 함
