    ...
    html += R"rawliteral(
        ...
        <script>
            ...
            function connectWs(){
                ...
                ws.onmessage = e => {
                    try { const d = JSON.parse(e.data); if (d.type === "wifi_scan_result") { updateWifiList(d.networks); } } catch(err) {}
                };
            ...
            function fetchStatus(){fetch("/api/wifistatus").then(r=>r.json()).then(d=>{let s=document.getElementById("wifi-status");d.connected?s.innerHTML="Connected to <b>"+d.ssid+"</b><br>IP Address: "+d.ip:s.textContent="Not connected. AP Mode is active."});}
            ...
            function connectToWifi(e){
                ...
                .then(data => {
                    ...
                    let pollCount = 0;
                    let maxPolls = 8; // 8 * 2s = 16s timeout
...
void WebManager::handleWifiStatusApi(AsyncWebServerRequest* request) {
    ...
    if (doc["connected"]) { doc["ssid"] = WiFi.SSID(); doc["ip"] = WiFi.localIP().toString(); }
...
void WebManager::handleDownloadOtaApi(AsyncWebServerRequest* request) {
    xTaskCreate(otaDownloadTask, "otaDownloadTask", 8192, this, 2, NULL);
...
void WebManager::setupLogBroadcaster() { Log::setWebSocketLogSender([this](const String& msg) { if (_isServerRunning) _ws.textAll(msg); }); }
